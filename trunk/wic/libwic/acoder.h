...
public:
	...
	*/
	...
	struct model_t {
		...
		value_type max;
		//!	\brief Математическое ожидание модуля кодируемой величины
		value_type abs_avg;
		...
		*/
		...
		//!	\brief Количество символов, закодированных или декодированных
		//!	с использованием этой модели
		/*!	Введён для вычисления математического ожидания _M.

			\note Счётчик обнуляется при каждом вызове acoder::encode_start()
			или acoder::decode_start(). Почти аналогичено _encoded_symbols,
			но может быть использован не только во время отладки.
			_encoded_symbols оставлена для совместимости.
		*/
		unsigned int _n;

		//!	\brief Математическое ожидание модуля символов в моделе
		/*!	<i>M<sub>n</sub> = (M<sub>n-1</sub> * (n - 1) + abs(v)) / n</i>

			\note Это поле заполняется <b>автоматически</b> при инициализации
			аркодера и затем поддерживается в актуальном состоянии.Начальное
			значение равно 0.
		*/
		double _abs_average;
	...
	*/
	...
	//!	\brief Математическое ожидание модуля кодируемой (декодируемой)
	//!	величины
	const double &abs_average(const sz_t &model_no) const;
...
private:
	...
	//!	\brief Стандартный файловый поток для вывода информации о
	//!	кодируемых и декодируемых коэффициентов
	std::ofstream _dbg_vals_stream;
	#endif
};
